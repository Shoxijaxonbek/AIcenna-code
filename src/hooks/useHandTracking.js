...
// function remap01(v,from,to){ return clamp((v-from)/(to-from)); }
// function expSmooth(prev,next,alpha){ return prev+(next-prev)*alpha; }
// function dist2D(a,b){ if(!a||!b) return 0; return Math.sqrt((a.x-b.x)**2+(a.y-b.y)**2); }
// function dist3D(a,b){ if(!a||!b) return 0; return Math.sqrt((a.x-b.x)**2+(a.y-b.y)**2+((a.z||0)-(b.z||0))**2); }

// const PINCH_CLOSED  = 0.02;
// const PINCH_OPEN    = 0.28;
// const PINCH_THRESH  = 0.28;
// const ALPHA_FAST    = 0.50;
// const ALPHA_SLOW    = 0.20;
// const CONFIRM       = 3;
...
//   // 👌 Thumb + pinky touch → model switch
//   const isThumbPinkyTouch = lm[4] && lm[20] && dist2D(lm[4], lm[20]) < 0.12;

//   // 🤙 DESELECT: o'rta barmoq uchi (lm[12]) + bosh barmoq uchi (lm[4]) yaqin
//   // Ko'rsatkich YUQORIDA bo'lishi shart emas — faqat o'rta + bosh barmoq tegishi
//   // Ring va pinky yig'ilgan bo'lsin (yoki ochiq farq qilmaydi)
...
//     // 2. SCALPEL
//     if (raw==='IDLE') {
//       if (h2 && h1.isPinch && h2.isIndexOnly) raw = 'SCALPEL';
//       else if (h2 && h2.isPinch && h1.isIndexOnly) raw = 'SCALPEL';
//     }

//     // 3. VOICE — ☝️ + 🖐
//     if (raw==='IDLE' && h2) {
//       if ((h1.isIndexOnly && h2.isOpen) || (h2.isIndexOnly && h1.isOpen)) {
//         raw = 'VOICE_GESTURE';
//       }
//     }
...
//     // 5. DESELECT — o'rta barmoq + bosh barmoq tegilsa (bitta qo'l)
//     // Ko'rsatkhdan keyingi o'rta barmoq + bosh barmoq yaqin
//     if (raw==='IDLE' && h1.isMiddleThumbTouch && !h2) raw = 'DESELECT';

//     // 6. EXPLODE_TOGGLE — ✊ fist
//     if (raw==='IDLE' && h1.isFist && !h2) raw = 'EXPLODE_TOGGLE';

//     // 7. MODEL SWITCH — thumb + pinky
//     if (raw==='IDLE' && h1.isThumbPinkyTouch && !h2) {
//       raw = h1.wrist.x < 0.5 ? 'NEXT_MODEL' : 'PREV_MODEL';
//     }
...
//     if (confirmed !== 'IDLE') log.debug(`✓ ${confirmed}`);

//     let scalpelHand = null;
//     if (confirmed==='SCALPEL') {
//       if (h2?.isIndexOnly) scalpelHand=h2;
//       else if (h1.isIndexOnly) scalpelHand=h1;
//     }
...
//       } catch(e) { if(!cancelled) setState(s=>({...s,status:'error',error:'Camera: '+e.message})); return; }
//       if (cancelled) { stream.getTracks().forEach(t=>t.stop()); return; }

//       const video=videoRef.current;
//       video.srcObject=stream; video.muted=true; video.playsInline=true;
//       try { await video.play(); } catch(e) { log.warn(e.message); }
//       await new Promise(r=>{if(video.readyState>=2){r();return;}video.addEventListener('loadeddata',r,{once:true});setTimeout(r,2000);});
//       if (cancelled) return;

//       try { await loadMediaPipe(); } catch(e) { if(!cancelled) setState(s=>({...s,status:'error',error:'MediaPipe: '+e.message})); return; }
//       if (cancelled) return;
...
//       } catch(e) { if(!cancelled) setState(s=>({...s,status:'error',error:'Hands: '+e.message})); return; }
//       if (cancelled) return;

//       const loop=async()=>{
//         if(cancelled)return;
//         try{const v=videoRef.current;if(v?.readyState>=2&&v.videoWidth>0)await handsRef.current.send({image:v});}catch(e){log.warn(e.message);}
