...
// Vibrant palette — 24 rang (texture o'tkazib bo'lmasa shu ishlatiladi)
const PALETTE = [
...
function getDisplayColor(rawData) {
  ...
  // If the raw color is not white/light, use it directly
  ...
  if (!isLight) return c;
  // White/light → use palette by meshIndex
  const idx = rawData.meshIndex ?? 0;
  return PALETTE[idx % PALETTE.length];
}

// ─── Mini vanilla Three.js renderer ──────────────────────────────────────────
function MiniRenderer({ meshRawData }) {
  ...
  useEffect(() => {
    ...
    // 30% larger than before (was 210×170)
    const W = 210, H = 221;
    ...
    const dl3 = new THREE.DirectionalLight(0xffffff, 0.4); dl3.position.set(0,0,5); scene.add(dl3);
    ...
    function setMesh(raw) {
      ...
      if (raw.indices?.length > 0) {
        const idxArr = raw.indices.length > 65535
    ...
    const animate = () => {
      ...
      angle += 0.009;
      if (mesh) {
        // Y o'qi (gorizontal) + X o'qi (vertikal) — tumbling
        mesh.rotation.y = angle;
        mesh.rotation.x = angle * 0.38;
      }
  ...
  useEffect(() => {
    stateRef.current?.setMesh(meshRawData);
  }, [meshRawData]);
...
// ─── Main export ──────────────────────────────────────────────────────────────
export default function PartDetailView({ partName, partInfo, onClose, onAskAI, meshRawData }) {
    ...
    <div style={{ position:'fixed', top:80, left:18, width:228, zIndex:300, animation:'pdvIn 0.22s ease', pointerEvents:'auto' }}>
      ...
      <div style={{ background:'rgba(0,3,14,0.98)', border:'1px solid rgba(0,212,255,0.4)', borderRadius:10, overflow:'hidden', boxShadow:'0 0 24px rgba(0,212,255,0.1)', position:'relative' }}>
        ...
        <div style={{ padding:'0 12px 10px' }}>
          {partInfo ? (
            <div style={{ fontSize:11,color:'rgba(188,215,255,0.85)',lineHeight:1.6,marginBottom:10,fontFamily:"'Segoe UI',sans-serif" }}>
              {partInfo}
            </div>
          ) : (
            <div style={{ fontSize:10,color:'rgba(255,255,255,0.18)',marginBottom:10,fontFamily:'monospace',fontStyle:'italic' }}>
              Ma'lumot yo'q
            </div>
          )}

          <div style={{ fontSize:8,color:'rgba(255,255,255,0.15)',fontFamily:'monospace',marginBottom:8 }}>
            🤙 O'rta+bosh barmoq → bekor
          </div>
          ...
          <button
            ...
            style={{ width:'100%',background:'rgba(0,212,255,0.08)',border:'1px solid rgba(0,212,255,0.35)',borderRadius:6,padding:'7px 0',color:'#00d4ff',fontSize:10,fontFamily:'monospace',cursor:'pointer',letterSpacing:'0.5px',transition:'background 0.15s' }}
          >
            🎤 Bu qism haqida so'rash
          </button>
        </div>
